				...
				<Card className="w-full md:w-3/4 lg:w-2/4 flex flex-col items-center justify-center bg-card text-card-foreground">
							...
							<form onSubmit={uploadForm.handleSubmit(submitFile)}>
											...
											<FormControl>
												<div {...getRootProps()}>
													{uploadForm.getValues("file") ? (
														<Label
															htmlFor="file"
															className="border border-dashed h-[256px] w-full flex gap-4 flex-col items-center justify-center rounded text-muted-foreground"
														>
															<FileArchive size={48} className="mr-4" />
															<div>
																<p className="text-center">
																	{uploadForm.getValues("file")?.name}
																</p>
																<p className="text-center">
																	{convertBytesToMb(
																		uploadForm.getValues("file")?.size ?? 0,
																	).toFixed(2)}{" "}
																	MB
																</p>
															</div>
															...
															{uploadForm.getValues("started") && (
																<>
																	<Progress
																		className="w-[60%] rounded mb-[-8px]"
																		value={uploadForm.getValues("percent")}
																	/>
																	<p className="text-center">
																		{uploadForm.getValues("percent")}%
																	</p>
																</>
															)}
														</Label>
														...
														<Label
															htmlFor="file"
															className="border border-dashed h-[256px] w-full flex items-center justify-center text-muted-foreground"
														>
								...
								<div className="mt-4 flex flex-row justify-end">
									...
									{uploadForm.getValues("shareUrl") && (
										<Button onClick={() => uploadForm.reset()}>
											Upload another file
										</Button>
									)}
									{!uploadForm.getValues("started") && (
										<Button
											disabled={uploadForm.getValues("isUploading")}
											type="submit"
										>
											Upload
										</Button>
					...
					{uploadForm.getValues("shareUrl") && (
						<CardFooter className="border rounded w-full flex items-center justify-center py-4">
							<Button variant="link" onClick={copyLinkToClipboard}>
								<Copy />
							</Button>
